...
- The Ainekio adapter can be started from the repo root with `./start-ainekio-adapter.sh`.
...
- Added `start-ainekio-adapter.sh` in the Ainekio repo root. It starts the adapter with default URL `http://192.168.0.44:4321` and session `ainekio-sim-1`, with environment-variable overrides.
...
- A previous Ainekio motion test run passed with `PYTHONPATH=motion/src python3 -m unittest discover -s motion/tests` before the bridge-output error work.
...
- Current stack command: `./start-ainekio-sim-stack.sh`.
- Stack starts:
