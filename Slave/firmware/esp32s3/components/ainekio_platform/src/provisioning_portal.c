...
static const char CONFIG_PAGE_INITIAL[] =
    ...
    "<p>Choose the WiFi used by the brain computer, then enter that computer's "
    "robot gateway address.</p><form method=post action=/configure>"
    ...
    "<label>Robot gateway<input name=endpoint_url maxlength=255 "
    "placeholder=\"ws://brain-lan-ip:8790/robot\" required><small>Use the brain "
    "computer's LAN address and the /robot path.</small></label>"
